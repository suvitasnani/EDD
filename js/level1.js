...
function handleEnd(evt) {
  ...
  if (
    Array.isArray(userStroke) &&
    userStroke.length > 0 &&
    Array.isArray(letterRef) &&
    letterRef.length > 0
  ) {
...
function generateLetterPoints(img) {
  ...
  const step = Math.max(
    2,
    Math.floor(Math.min(canvas.width, canvas.height) / 200)
  );
  ...
  console.log(
    `Generated ${currentLetterPoints.length} reference points for letter.`
  );
}
...
async function evaluateLetter(goodArray, userArray) {
  ...
  ); // HOW STRICT IT IS, NOTE TO SELF TO TUNE IT

  for (let i = 0; i < userArray.length; i++) {
    let minDistance = Infinity;
    for (let j = 0; j < goodArray.length; j++) {
      let distance = Math.sqrt(
        (userArray[i][0] - goodArray[j][0]) *
          (userArray[i][0] - goodArray[j][0]) +
          (userArray[i][1] - goodArray[j][1]) *
            (userArray[i][1] - goodArray[j][1])
      );
    ...
    let pointsForThisPixel = Math.max(
      0,
      (maxAcceptableDistance - minDistance) / maxAcceptableDistance
    );
  ...
  if (tempPoints > 0.7) {
    ...
    if (gif && gif.classList) {
      gif.classList.replace(
        `lvl1image${currentLetter - 1}`,
        `lvl1image${currentLetter}`
      );
    }
...
async function setScore(score) {
  ...
  let points = 0;
  await get(child(dbref, "users/" + currentUser.uid + "/points"))
    .then((snapshot) => {
      if (snapshot.exists()) {
        points = snapshot.val();
      } else {
  ...
  await update(ref(db, "users/" + currentUser.uid), {
    ...
    .then(() => {
      alert(
        `Congragulations! You scored ${score}% and received ${
          points - ogpoints
        } points!`
      );
